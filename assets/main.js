const API_BASE_URL = "https://apiscore-2eqe.onrender.com";
...
async function initHomePage() {
  ...
  try {
    ...
    const played = matches.filter((m) =>
      ...
      m.status === "played" && new Date(m.game_date) <= now

    );

    const scheduled = matches.filter((m) =>

      m.status === "scheduled" && new Date(m.game_date) >= now
...
function updateHomePage(nextMatch, lastMatch) {
  ...
  if (nextMatch) {
    ...
    nextDiv.innerHTML = `
      ...
      <p>Date : ${formatMatchDate(nextMatch.game_date)}</p>
  ...
  if (lastMatch) {
    ...
    lastDiv.innerHTML = `
      ...
      <p>Date : ${formatMatchDate(lastMatch.game_date)}</p>
      <p>Score : ${score}</p>
...
function renderMatchesTable(matches) {
  ...
  matches.sort((a, b) => new Date(a.game_date) - new Date(b.game_date));

  matches.forEach((match) => {
    const tr = document.createElement("tr");

    const dateTd = document.createElement("td");
    dateTd.textContent = formatMatchDate(match.game_date);
